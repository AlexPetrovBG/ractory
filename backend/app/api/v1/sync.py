...
async def sync_projects(
    ...
    Requires API key with sync:true scope or admin role.
    """
    # Validate access (admin or API key with sync scope)
    if current_user.role not in ["SystemAdmin", "CompanyAdmin", "Integration"]:
        if not current_user.extras.get("scope", {}).get("sync"):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="Insufficient permissions for sync operations"
            )
    
    # Perform bulk upsert using service
    result = await SyncService.upsert_projects(data.projects, current_user.tenant, session)
    
    return SyncResult(**result)

@router.post("/components", response_model=SyncResult)
...
@router.post("/assemblies", response_model=SyncResult)
...
@router.post("/pieces", response_model=SyncResult)
...
@router.post("/articles", response_model=SyncResult)
